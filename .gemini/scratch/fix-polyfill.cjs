...
['index.html', 'bot-tiktok.html', 'phonefarm.html', 'proxy-finder.html'].forEach(file => {
    ...
    // Regex to match the old polyfill
    const oldPolyfillRegex = /\/\/ POLYFILL TO PREVENT JS CRASHES ON MISSING ELEMENTS[\s\S]*?return el;\s*};/;
    
    if (content.match(oldPolyfillRegex)) {
        content = content.replace(oldPolyfillRegex, polyfill.trim());
        fs.writeFileSync('public/' + file, content);
    }
});
console.log('Targeted Polyfill injected.');
